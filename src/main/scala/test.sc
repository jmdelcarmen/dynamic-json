...
def parse[T](data: String): Any = {
  def _parse(data: List[Char], acc: List[Accumulator[T]]): Any = data match {
          ...
          def composeMap(currentKey: Option[Accumulator[T]],
            ...
            if (accumulators.isEmpty) map
              ...
              if (currentValue.isDefined && currentKey.isDefined) {
                ...
                composeMap(None, None, acc, nextMap.asInstanceOf[T])
              } else {
                val nextAcc :: rest = accumulators
                // Set next acc

                if (nextAcc.isKey) composeMap(Some(nextAcc), None, rest, map)
                else composeMap(None, Some(nextAcc), rest, map)
              }
            }
          }
